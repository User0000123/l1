...
void DrawTriangleIl(PARAMS *pThreadParams)
{
...
int PrepareTriangleFillData(PARAMS *pThreadParams, int index)
{
  gsl_vector_int *pTriangleVertices = pObjFile->fv->data[index];
  if (!(pIsDrawable[pTriangleVertices->data[0]] && pIsDrawable[pTriangleVertices->data[1]] && pIsDrawable[pTriangleVertices->data[2]]))
    return 1;

  gsl_vector_memcpy(pThreadParams->pVN0, pObjFile->vn->data[pTriangleVertices->data[0]]);
  gsl_vector_memcpy(pThreadParams->pVN1, pObjFile->vn->data[pTriangleVertices->data[1]]);
  gsl_vector_memcpy(pThreadParams->pVN2, pObjFile->vn->data[pTriangleVertices->data[2]]);
  ...
  return ((abs(pThreadParams->pV0->data[1] - pThreadParams->pV1->data[1]) < 1) && (abs(pThreadParams->pV0->data[1] - pThreadParams->pV2->data[1]) < 1));
}
